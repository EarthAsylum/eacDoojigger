...
$this->registerExtensionOptions( $this->className,
	[
		'secCorsOpt' 	=> array(
				...
				'options'	=>	array(
					...
					["Validate this site's <abbr title='".$this->plugin->varServer('host')."'>origin</abbr> to its <abbr title='Helps block origin spoofing'>IP address</abbr>" => 'host_origin'],
				),
	...
	//			'attributes'=>	(!is_network_admin() && $this->isNetworkPolicy('secCorsOpt')) ? 'disabled="disabled"' : '',
		...
		'secAllowCors' 	=> array(
				'type'		=>	'textarea',
				'label'		=>	"CORS <abbr title='Application Program Interface'>API</abbr> Whitelist",
