 ...
 * use this trait:
 *		use \EarthAsylumConsulting\Traits\plugin_loader_environment
 *
 ...
 *				// if USEing plugin_loader_environment trait
...
trait plugin_loader_environment
{
	...
	private static function check_loader_environment(): bool
	{
		...
		\add_action('current_screen', function($screen)
			{
				$pluginName = basename(dirname(self::$plugin_detail['PluginFile']));
				// on  plugins  page or this plugin's page
				if  ($screen->base  == 'plugins' || stristr($screen->base, $pluginName))
				{
					\add_action( (is_network_admin() ? 'network_' : '').'admin_notices', function()
						{
							echo  '<div class="notice notice-warning is-dismissible"><p><strong>' .
								sprintf("Warning from %s: This plugin is using a deprecated trait that will not be supported in future versions.",
										plugin_basename(self::$plugin_detail['PluginFile']),
								) . "</strong></p><p>" .
								sprintf("Please replace %s with %s.",
										"<em>\"plugin_loader_environment\"</em> trait", "<em>\"plugin_environment\"</em> trait"
								) . "</p>" .
								"</div>";
						}
					);
	...
	private static function output_version_notice(string $moduleName, string $required, string $version): void
	{
		...
		echo  '<div class="notice notice-error is-dismissible"><h4>' .
